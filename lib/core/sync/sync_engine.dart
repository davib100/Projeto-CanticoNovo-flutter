...
class SyncEngine {
  ...
  Future<SyncResult> sync({
    ...
    try {
      ...
      final result = SyncResult(
        ...
        conflictsResolved: pushResult.conflictsCount + pullResult.conflictsCount,
            ...
            if (opResult.hasConflict) {
              ...
              if (resolved) {
                ...
                errors.add(SyncError(
                  message: 'Unresolved conflict for ${opResult.localOperation.id}',
                  timestamp: DateTime.now(),
                ));
              }
      ...
      await _db.transaction((txn) async {
        for (int i = 0; i < serverOperations.length; i++) {
          ...
          try {
            ...
            if (localVersion != null) {
              ...
              if (serverOp.version <= localVersion.version) {
                ...
                if (resolved) {
                  ...
                  errors.add(SyncError(
                    message: 'Version conflict for ${serverOp.entityType}:${serverOp.entityId}',
                    timestamp: DateTime.now(),
                  ));
                  continue;
                }
            ...
            _updateState(SyncState.syncing(
              progress: progress,
              currentOperation: 'Applying ${i + 1}/${serverOperations.length}',
            ));
  ...
  Future<List<SyncOperation>> _loadPendingOperations({
    ...
    if (entityTypes != null && entityTypes.isNotEmpty) {
      whereConditions.add('entity_type IN (${List.filled(entityTypes.length, '?').join(',')})');
  ...
  Future<BatchResponse> _sendBatchToServer(
  ...
  Future<void> _cleanupSyncedOperations() async {
    ...
    await _db.deleteGeneric(
      'sync_log',
      'status = ? AND synced_at < ?',
      ['synced', cutoffDate.toIso8601String()],
    );
  }
